...
function executeInTabIfBlocked(action, tab) {
  ...
  if (PREFS.selectedIndex === 1 && action === 'block') {
    action = 'unblock'; // Use 'unblock' action for physical rest mode
  }
...
function blockWebsites() {
  chrome.tabs.query({}, function (tabs) {
    for (var i = 0; i < tabs.length; i++) {
      //chrome.tabs.update(tabs[i].id, { url: 'chrome-extension://your_extension_id/blocked.html' });
    }
  });
}
